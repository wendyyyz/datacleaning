...
'''def extract_coordinate(loc):
    coordinates = re.findall("\(.+\)", loc)
    coor_list = coordinates[0].split(",")
    lat = coor_list[0].replace("(", "")
    return lat

data["hs_directory"]["lat"] = data["hs_directory"]["Location1"].apply(extract_coordinate)
'''
